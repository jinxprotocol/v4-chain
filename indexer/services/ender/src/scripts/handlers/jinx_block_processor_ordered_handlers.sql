CREATE OR REPLACE FUNCTION jinx_block_processor_ordered_handlers(block jsonb) RETURNS jsonb[] AS $$
/**
        ...
        CASE event_->'subtype'
            WHEN '"market"'::jsonb THEN
                IF event_data->'priceUpdate' IS NOT NULL THEN
                    rval[i] = jinx_market_price_update_handler(block_height, block_time, event_data);
                ELSIF event_data->'marketCreate' IS NOT NULL THEN
                    rval[i] = jinx_market_create_handler(event_data);
                ELSIF event_data->'marketModify' IS NOT NULL THEN
                    rval[i] = jinx_market_modify_handler(event_data);
                ELSE
                    RAISE EXCEPTION 'Unknown market event %', event_;
                END IF;
            WHEN '"asset"'::jsonb THEN
                rval[i] = jinx_asset_create_handler(event_data);
            WHEN '"perpetual_market"'::jsonb THEN
                rval[i] = jinx_perpetual_market_handler(event_data);
            WHEN '"liquidity_tier"'::jsonb THEN
                rval[i] = jinx_liquidity_tier_handler(event_data);
            WHEN '"update_perpetual"'::jsonb THEN
                rval[i] = jinx_update_perpetual_handler(event_data);
            WHEN '"update_clob_pair"'::jsonb THEN
                rval[i] = jinx_update_clob_pair_handler(event_data);
            ELSE
                NULL;
            END CASE;
    END LOOP;
