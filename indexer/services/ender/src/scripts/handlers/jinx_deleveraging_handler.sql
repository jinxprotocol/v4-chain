CREATE OR REPLACE FUNCTION jinx_deleveraging_handler(
...
/**
  ...
  Returns: JSON object containing fields:
    - liquidated_fill: The created liquidated fill in fill-model format (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/fill-model.ts).
    - offsetting_fill: The created offsetting fill in fill-model format (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/fill-model.ts).
    - perpetual_market: The perpetual market for the deleveraging in perpetual-market-model format (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/perpetual-market-model.ts).
    - liquidated_perpetual_position: The updated liquidated perpetual position in perpetual-position-model format (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/perpetual-position-model.ts).
    - offsetting_perpetual_position: The updated offsetting perpetual position in perpetual-position-model format (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/perpetual-position-model.ts).
...
BEGIN
    ...
    */
    size = jinx_trim_scale(jinx_from_jsonlib_long(event_data->'fillAmount') *
                                 power(10, perpetual_market_record."atomicResolution")::numeric);
    price = jinx_trim_scale(jinx_from_jsonlib_long(event_data->'price') *
                                  power(10, QUOTE_CURRENCY_ATOMIC_RESOLUTION)::numeric);

    liquidated_subaccount_uuid = jinx_uuid_from_subaccount_id(event_data->'liquidated');
    offsetting_subaccount_uuid = jinx_uuid_from_subaccount_id(event_data->'offsetting');
    ...
    VALUES (jinx_uuid_from_fill_event_parts(event_id, 'TAKER'),
    ...
    INSERT INTO fills
        ("id", "subaccountId", "side", "liquidity", "type", "clobPairId", "size", "price", "quoteAmount",
         "eventId", "transactionHash", "createdAt", "createdAtHeight", "fee")
    VALUES (jinx_uuid_from_fill_event_parts(event_id, 'MAKER'),
    ...
    liquidated_perpetual_position_record = jinx_update_perpetual_position_aggregate_fields(
        liquidated_subaccount_uuid,
        perpetual_id,
        liquidated_side,
        size,
        price);
    offsetting_perpetual_position_record = jinx_update_perpetual_position_aggregate_fields(
        offsetting_subaccount_uuid,
        perpetual_id,
        offsetting_side,
        size,
    ...
    RETURN jsonb_build_object(
            'liquidated_fill',
            jinx_to_jsonb(liquidated_fill_record),
            'offsetting_fill',
            jinx_to_jsonb(offsetting_fill_record),
            'perpetual_market',
            jinx_to_jsonb(perpetual_market_record),
            'liquidated_perpetual_position',
            jinx_to_jsonb(liquidated_perpetual_position_record),
            'offsetting_perpetual_position',
            jinx_to_jsonb(offsetting_perpetual_position_record)
        );
END;
$$ LANGUAGE plpgsql;
