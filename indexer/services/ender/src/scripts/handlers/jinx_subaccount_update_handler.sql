CREATE OR REPLACE FUNCTION jinx_subaccount_update_handler(
...
/**
  ...
  Returns: JSON object containing fields:
    - subaccount: The upserted subaccount in subaccount-model format (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/subaccount-model.ts).
    - perpetual_positions: A JSON array of upserted perpetual positions in perpetual-position-model format (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/perpetual-position-model.ts).
    - asset_positions: A JSON array of upserted asset positions in asset-position-model format (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/asset-position-model.ts).
...
BEGIN
    ...
    subaccount_record."id" = jinx_uuid_from_subaccount_id(event_data->'subaccountId');
    ...
    FOR perpetual_position_update IN SELECT * FROM jsonb_array_elements(event_data->'updatedPerpetualPositions') LOOP
        ...
        BEGIN
            ...
            _size = jinx_trim_scale(jinx_from_serializable_int(perpetual_position_update->'quantums') *
            ...
            settled_funding = jinx_trim_scale(-jinx_from_serializable_int(perpetual_position_update->'fundingPayment')
            ...
            IF perpetual_position_found AND (_size = 0 OR perpetual_position_record.side != side) THEN
                ...
                perpetual_position_record_updates = array_append(
                    perpetual_position_record_updates, jinx_to_jsonb(perpetual_position_record));
            ...
            IF NOT perpetual_position_found
                ...
                perpetual_position_record."id" = jinx_uuid_from_perpetual_position_parts(subaccount_record.id, event_id);
    ...
    FOR asset_position_update IN SELECT * FROM jsonb_array_elements(event_data->'updatedAssetPositions') LOOP
        ...
        BEGIN
            ...
            size = jinx_trim_scale(jinx_from_serializable_int(asset_position_update->'quantums') *
                   power(10, asset_record."atomicResolution")::numeric);
            asset_position_record.id = jinx_uuid_from_asset_position_parts(subaccount_record.id, asset_id);
            ...
            asset_position_record_updates = array_append(
                asset_position_record_updates, jinx_to_jsonb(asset_position_record));
        END;
    END LOOP;

    RETURN jsonb_build_object(
        'subaccount', jinx_to_jsonb(subaccount_record),
