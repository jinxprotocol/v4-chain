CREATE OR REPLACE FUNCTION jinx_funding_handler(
...
/**
  ...
  Returns: JSON object containing fields:
    - perpetual_markets: A mapping from perpetual market id to the associated perpetual market in perpetual-market-model format (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/perpetual-market-model.ts).
...
BEGIN
    FOR funding_update IN SELECT * FROM jsonb_array_elements(event_data->'updates') LOOP
        ...
        perpetual_markets_response = jsonb_set(perpetual_markets_response, ARRAY[(perpetual_market_record."id")::text], jinx_to_jsonb(perpetual_market_record));
        ...
        CASE event_data->'type'
            ...
            WHEN TYPE_FUNDING_RATE_AND_INDEX THEN
                ...
                event_id = jinx_event_id_from_parts(block_height, transaction_index, event_index);

                funding_index_updates_record."id" = jinx_uuid_from_funding_index_update_parts(
                    block_height,
                    event_id,
                ...
                funding_index_updates_record."rate" = jinx_trim_scale(
                ...
                funding_index_updates_record."fundingIndex" = jinx_trim_scale(
                    jinx_from_serializable_int(funding_update->'fundingIndex') *
                    power(10,
                ...
                funding_update_response = jsonb_set(funding_update_response, ARRAY[(funding_index_updates_record."perpetualId")::text], jinx_to_jsonb(funding_index_updates_record));
