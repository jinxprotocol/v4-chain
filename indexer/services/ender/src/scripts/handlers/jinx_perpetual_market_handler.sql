CREATE OR REPLACE FUNCTION jinx_perpetual_market_handler(event_data jsonb) RETURNS jsonb AS $$
/**
  Parameters:
  ...
  Returns: JSON object containing fields:
    - perpetual_market: The updated perpetual market in perpetual-market-model format (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/perpetual-market-model.ts).
...
BEGIN
    ...
    perpetual_market_record."status" = jinx_clob_pair_status_to_market_status(event_data->'status');
    ...
    perpetual_market_record."stepBaseQuantums" = jinx_from_jsonlib_long(event_data->'stepBaseQuantums');
