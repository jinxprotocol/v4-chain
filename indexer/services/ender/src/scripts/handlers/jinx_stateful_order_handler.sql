CREATE OR REPLACE FUNCTION jinx_stateful_order_handler(
...
/**
  ...
  Returns: JSON object containing fields:
    - order: The upserted order in order-model format (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/order-model.ts).
...
BEGIN
    ...
    IF event_data->'orderPlace' IS NOT NULL OR event_data->'longTermOrderPlacement' IS NOT NULL OR event_data->'conditionalOrderPlacement' IS NOT NULL THEN
        ...
        */
        order_record."id" = jinx_uuid_from_order_id(order_->'orderId');
        order_record."subaccountId" = jinx_uuid_from_subaccount_id(order_->'orderId'->'subaccountId');
        order_record."clientId" = jsonb_extract_path_text(order_, 'orderId', 'clientId')::bigint;
        order_record."clobPairId" = clob_pair_id;
        order_record."side" = jinx_from_protocol_order_side(order_->'side');
        order_record."size" = jinx_trim_scale(jinx_from_jsonlib_long(order_->'quantums') *
                                              power(10, perpetual_market_record."atomicResolution")::numeric);
        order_record."totalFilled" = 0;
        order_record."price" = jinx_trim_scale(jinx_from_jsonlib_long(order_->'subticks') *
        ...
        CASE
            WHEN event_data->'conditionalOrderPlacement' IS NOT NULL THEN
                order_record."type" = jinx_protocol_condition_type_to_order_type(order_->'conditionType');
                order_record."status" = 'UNTRIGGERED';
                order_record."triggerPrice" = jinx_trim_scale(jinx_from_jsonlib_long(order_->'conditionalOrderTriggerSubticks') *
                                                              power(10, perpetual_market_record."quantumConversionExponent" +
                                                                        QUOTE_CURRENCY_ATOMIC_RESOLUTION -
                                                                        perpetual_market_record."atomicResolution")::numeric);
            ELSE
    ...
    ELSIF event_data->'conditionalOrderTriggered' IS NOT NULL OR event_data->'orderRemoval' IS NOT NULL THEN
        ...
        subaccount_id = jinx_uuid_from_subaccount_id(order_id->'subaccountId');
        ...
        order_record."id" = jinx_uuid_from_order_id(order_id);
        order_record."updatedAt" = block_time;
        order_record."updatedAtHeight" = block_height;
        UPDATE orders
        SET
        ...
        IF NOT FOUND THEN
            RAISE EXCEPTION 'Unable to update order status with orderId: %', jinx_uuid_from_order_id(order_id);
        END IF;
        ...
        RETURN jsonb_build_object(
                ...
                jinx_to_jsonb(perpetual_market_record),
                'subaccount',
                jinx_to_jsonb(subaccount_record)
            );
    ELSE
