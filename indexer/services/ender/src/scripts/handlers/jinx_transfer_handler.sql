CREATE OR REPLACE FUNCTION jinx_transfer_handler(
...
/**
  ...
  Returns: JSON object containing fields:
    - asset: The existing asset in asset-model format (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/asset-model.ts).
    - transfer: The new transfer in transfer-model format (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/transfer-model.ts).
...
BEGIN
    ...
    IF event_data->'recipient'->'subaccountId' IS NOT NULL THEN
        transfer_record."recipientSubaccountId" = jinx_uuid_from_subaccount_id(event_data->'recipient'->'subaccountId');
    ...
    IF event_data->'sender'->'subaccountId' IS NOT NULL THEN
        transfer_record."senderSubaccountId" = jinx_uuid_from_subaccount_id(event_data->'sender'->'subaccountId');
    END IF;
    ...
    transfer_record."size" = jinx_trim_scale(jinx_from_jsonlib_long(event_data->'amount') * power(10, asset_record."atomicResolution")::numeric);
    transfer_record."eventId" = jinx_event_id_from_parts(block_height, transaction_index, event_index);
    ...
    transfer_record."id" = jinx_uuid_from_transfer_parts(
    ...
    RETURN jsonb_build_object(
        'asset',
        jinx_to_jsonb(asset_record),
        'transfer',
        jinx_to_jsonb(transfer_record)
    );
END;
$$ LANGUAGE plpgsql;
