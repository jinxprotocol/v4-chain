CREATE OR REPLACE FUNCTION jinx_tendermint_event_to_transaction_index(event jsonb) RETURNS int AS $$
/**
  Gets the transaction index from the IndexerTendermint event.

  Parameters:
    - event: The JSON.stringify of a IndexerTendermintEvent object (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/proto/jinxprotocol/indexer/indexer_manager/event.proto#L25).
  Returns: int.
