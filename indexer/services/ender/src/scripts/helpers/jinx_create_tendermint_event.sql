CREATE OR REPLACE FUNCTION jinx_create_tendermint_event(
...
BEGIN
    transaction_idx := jinx_tendermint_event_to_transaction_index(event);
    event_id := jinx_event_id_from_parts(CAST(block_height AS int), transaction_idx, CAST(event->>'eventIndex' AS int));
