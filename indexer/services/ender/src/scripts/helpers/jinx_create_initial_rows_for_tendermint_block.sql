CREATE OR REPLACE FUNCTION jinx_create_initial_rows_for_tendermint_block(
...
BEGIN
    ...
    IF tx_hashes IS NOT NULL AND jsonb_array_length(tx_hashes) > 0 THEN
        FOR i IN 0..jsonb_array_length(tx_hashes)-1 LOOP
            PERFORM jinx_create_transaction(jsonb_array_element_text(tx_hashes, i), block_height, i);
        END LOOP;
    END IF;
    ...
    IF events IS NOT NULL AND jsonb_array_length(events) > 0 THEN
        FOR i IN 0..jsonb_array_length(events)-1 LOOP
            PERFORM jinx_create_tendermint_event(jsonb_array_element(events, i), block_height);
        END LOOP;
    END IF;
END;
$$ LANGUAGE plpgsql;
