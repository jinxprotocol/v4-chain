CREATE OR REPLACE FUNCTION jinx_process_trading_reward_event(
...
/**
  Parameters:
    - trading_reward: the trading reward to process, which should match AddressTradingReward (proto/jinxprotocol/indexer/events/events.proto#AddressTradingReward).
...
BEGIN
    amount_in_human_readable = jinx_trim_scale(
      jinx_from_serializable_int(trading_reward->'denomAmount') * power(10, DENOM_TO_COIN_EXPONENT)::numeric
    );
    ...
    trading_reward_record."id" = jinx_uuid_from_trading_rewards_parts(trading_reward->>'owner', block_height);
    ...
    RETURN jinx_to_jsonb(trading_reward_record);
END;
$$ LANGUAGE plpgsql;
