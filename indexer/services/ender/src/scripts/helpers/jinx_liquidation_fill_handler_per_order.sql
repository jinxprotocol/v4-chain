CREATE OR REPLACE FUNCTION jinx_liquidation_fill_handler_per_order(
...
/**
  Parameters:
    ...
    - event_data: The 'data' field of the IndexerTendermintEvent (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/proto/jinxprotocol/indexer/indexer_manager/event.proto#L25)
    ...
    - transaction_index: The transaction_index of the IndexerTendermintEvent after the conversion that takes into
        account the block_event (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/indexer/services/ender/src/lib/helper.ts#L41)
  ...
  Returns: JSON object containing fields:
    - order: The updated order in order-model format (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/order-model.ts).
        Only returned if field == 'makerOrder'.
    - fill: The updated fill in fill-model format (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/fill-model.ts).
    - perpetual_market: The perpetual market for the order in perpetual-market-model format (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/perpetual-market-model.ts).
    - perpetual_position: The updated perpetual position in perpetual-position-model format (https://github.com/jinxprotocol/v4-chain/blob/9ed26bd/indexer/packages/postgres/src/models/perpetual-position-model.ts).
...
BEGIN
    ...
    perpetual_market_record = jinx_get_perpetual_market_for_clob_pair(clob_pair_id);
    ...
    */
    fill_amount = jinx_trim_scale(jinx_from_jsonlib_long(event_data->'fillAmount') *
                                  power(10, perpetual_market_record."atomicResolution")::numeric);
    maker_price = jinx_trim_scale(jinx_from_jsonlib_long(maker_order->'subticks') *
    ...
    total_filled = jinx_trim_scale(jinx_get_total_filled(fill_liquidity, event_data) *
                                   power(10, perpetual_market_record."atomicResolution")::numeric);
    fee = jinx_trim_scale(jinx_get_fee(fill_liquidity, event_data) *
                          power(10, asset_record."atomicResolution")::numeric);
    order_price = jinx_trim_scale(jinx_from_jsonlib_long(order_->'subticks') *
                                  power(10, perpetual_market_record."quantumConversionExponent" +
                                            asset_record."atomicResolution" -
                                            perpetual_market_record."atomicResolution")::numeric);

    IF field = 'makerOrder' THEN
        order_uuid = jinx_uuid_from_order_id(order_->'orderId');
        subaccount_uuid = jinx_uuid_from_subaccount_id(jsonb_extract_path(order_, 'orderId', 'subaccountId'));
        order_client_metadata = (order_->'clientMetadata')::bigint;
        order_side = jinx_from_protocol_order_side(order_->'side');
    ELSE
        order_uuid = NULL;
        subaccount_uuid = jinx_uuid_from_subaccount_id(jsonb_extract_path(order_, 'liquidated'));
    ...
    IF field = 'makerOrder' THEN
        order_size = jinx_trim_scale(jinx_from_jsonlib_long(order_->'quantums') *
                                     power(10, perpetual_market_record."atomicResolution")::numeric);
        ...
        order_record."timeInForce" = jinx_from_protocol_time_in_force(order_->'timeInForce');
        ...
        IF FOUND THEN
            ...
            order_record."status" = jinx_get_order_status(total_filled, order_record.size, 'NOT_CANCELED', order_record."orderFlags", order_record."timeInForce");
        ...
        ELSE
            ...
            order_record."status" = jinx_get_order_status(fill_amount, order_size, 'NOT_CANCELED', order_record."orderFlags", order_record."timeInForce");
    ...
    event_id = jinx_event_id_from_parts(
    ...
    VALUES (jinx_uuid_from_fill_event_parts(event_id, fill_liquidity),
            subaccount_uuid,
            order_side,
            fill_liquidity,
            fill_type,
            ...
            fill_amount,
            maker_price,
            jinx_trim_scale(fill_amount * maker_price),
            event_id,
            transaction_hash,
            block_time,
            block_height,
    ...
    BEGIN
        IF jinx_perpetual_position_and_order_side_matching(
                perpetual_position_record."side", order_side) THEN
            sum_open = jinx_trim_scale(perpetual_position_record."sumOpen" + fill_amount);
            entry_price = jinx_get_weighted_average(
        ...
        ELSE
            sum_close = jinx_trim_scale(perpetual_position_record."sumClose" + fill_amount);
            exit_price = jinx_get_weighted_average(
    ...
    IF field = 'makerOrder' THEN
        RETURN jsonb_build_object(
                'order',
                jinx_to_jsonb(order_record),
                'fill',
                jinx_to_jsonb(fill_record),
                'perpetual_market',
                jinx_to_jsonb(perpetual_market_record),
                'perpetual_position',
                jinx_to_jsonb(perpetual_position_record)
            );
    ELSE
        RETURN jsonb_build_object(
                'fill',
