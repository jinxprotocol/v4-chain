CREATE OR REPLACE FUNCTION jinx_update_perpetual_position_aggregate_fields(
...
BEGIN
    ...
    IF jinx_perpetual_position_and_order_side_matching(perpetual_position_record."side", side) THEN
        sum_open := jinx_trim_scale(perpetual_position_record."sumOpen" + size);
    ...
    ELSE
        sum_close := jinx_trim_scale(perpetual_position_record."sumClose" + size);
