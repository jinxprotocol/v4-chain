CREATE OR REPLACE FUNCTION jinx_block_processor(block jsonb) RETURNS jsonb AS $$
/**
...
DECLARE
    ...
    rval_to_merge jsonb[];
BEGIN
    PERFORM jinx_create_initial_rows_for_tendermint_block(block_height, block_time, block->'txHashes', block->'events');
    ...
    IF NOT block_height = 0 THEN
        rval = jinx_block_processor_unordered_handlers(block);
        rval_to_merge = jinx_block_processor_ordered_handlers(block);
    ELSE
        rval = jinx_block_processor_ordered_handlers(block);
        rval_to_merge = jinx_block_processor_unordered_handlers(block);
    END IF;
