...
describe('SQL Function Tests', () => {
  ...
  const events: IndexerTendermintEvent[] = [
    createIndexerTendermintEvent(
      JinxIndexerSubtypes.FUNDING,
      defaultMarketEventBinary,
      -1,
      eventIndex0,
    ),
    createIndexerTendermintEvent(
      JinxIndexerSubtypes.SUBACCOUNT_UPDATE,
  ...
  it.each([
    ...
    [1, 2, 3],
    [9, 8, 7],
  ])('jinx_event_id_from_parts (%d, %d, %d)', async (blockHeight: number, transactionIndex: number, eventIndex: number) => {
    const result = await getSingleRawQueryResultRow(`SELECT jinx_event_id_from_parts(${blockHeight}, ${transactionIndex}, ${eventIndex}) AS result;`);
  ...
  it.each([
    ...
    Long.fromNumber(-1_000_000_000, false),
  ])('jinx_from_jsonlib_long (%s)', async (value: Long) => {
    const result = await getSingleRawQueryResultRow(`SELECT jinx_trim_scale(jinx_from_jsonlib_long('${JSON.stringify(value)}')) AS result`);
  ...
  ])('jinx_from_protocol_order_side(%s)', async (_name: string, value: IndexerOrder_Side) => {
    const result = await getSingleRawQueryResultRow(`SELECT jinx_from_protocol_order_side('${value}') AS result`);
  ...
  ])('jinx_from_protocol_time_in_force (%s)', async (_name: string, value: IndexerOrder_TimeInForce) => {
    const result = await getSingleRawQueryResultRow(`SELECT jinx_from_protocol_time_in_force('${value}') AS result`);
  ...
  ])('jinx_protocol_condition_type_to_order_type (%s)', async (_name: string, value: IndexerOrder_ConditionType) => {
    const result = await getSingleRawQueryResultRow(`SELECT jinx_protocol_condition_type_to_order_type('${value}') AS result`);
  ...
  ])('jinx_from_serializable_int (%s)', async (value: string) => {
    const byteArray = bigIntToBytes(BigInt(value));
    const result = await getSingleRawQueryResultRow(
      `SELECT jinx_trim_scale(jinx_from_serializable_int('${JSON.stringify(byteArray)}')) AS result`);
  ...
  ])('jinx_get_weighted_average (%s)', async (_name: string, firstPrice: number | null, firstWeight: number, secondPrice: number | null, secondWeight: number) => {
    const result = await getSingleRawQueryResultRow(`SELECT jinx_get_weighted_average(${JSON.stringify(firstPrice)}, ${firstWeight}, ${JSON.stringify(secondPrice)}, ${secondWeight}) AS result`);
  ...
  ])('jinx_perpetual_position_and_order_side_matching (%s, %s)', async (perpSide: PositionSide, orderSide: OrderSide) => {
    const result = await getSingleRawQueryResultRow(`SELECT jinx_perpetual_position_and_order_side_matching('${perpSide}', '${orderSide}') AS result;`);
  ...
  it.each([
    ...
    ['-1.2300', '-1.23'],
  ])('jinx_trim_scale (%s)', async (value: string, expected) => {
    const result = await getSingleRawQueryResultRow(`SELECT jinx_trim_scale('${value}') AS result;`);
  ...
  it.each([
    'foo',
    'bar',
  ])('jinx_uuid (%s)', async (value: string) => {
    const result = await getSingleRawQueryResultRow(`SELECT jinx_uuid('${value}') AS result`);
  ...
  ])('jinx_uuid_from_asset_position_parts (%s)', async (subaccountId: IndexerSubaccountId, assetId: string) => {
    const subaccountUuid = SubaccountTable.subaccountIdToUuid(subaccountId);
    const result = await getSingleRawQueryResultRow(
      `SELECT jinx_uuid_from_asset_position_parts('${subaccountUuid}', '${assetId}') AS result`);
  ...
  ])('jinx_uuid_from_fill_event_parts (%s)', async (liquidity: Liquidity, blockHeight: number, transactionIndex: number, eventIndex: number) => {
    const eventId = TendermintEventTable.createEventId(`${blockHeight}`, transactionIndex, eventIndex);
    const result = await getSingleRawQueryResultRow(
      `SELECT jinx_uuid_from_fill_event_parts('\\x${eventId.toString('hex')}'::bytea, '${liquidity}') AS result`);
  ...
  it.each([
    [1, 2, 3, 4],
    [5, 6, 7, 8],
  ])('jinx_uuid_from_funding_index_update_parts (%s, %s, %s, %s)', async (blockHeight: number, transactionIndex: number, eventIndex: number, perpetualId: number) => {
    const eventId = TendermintEventTable.createEventId(`${blockHeight}`, transactionIndex, eventIndex);
    const result = await getSingleRawQueryResultRow(
      `SELECT jinx_uuid_from_funding_index_update_parts('${blockHeight}', '\\x${eventId.toString('hex')}'::bytea, '${perpetualId}') AS result`);
  ...
  ])('jinx_uuid_from_order_id and parts (%s)', async (orderId: IndexerOrderId) => {
    let result = await getSingleRawQueryResultRow(`SELECT jinx_uuid_from_order_id('${JSON.stringify(orderId)}') AS result`);
    expect(result).toEqual(OrderTable.orderIdToUuid(orderId));

    result = await getSingleRawQueryResultRow(`SELECT jinx_uuid_from_order_id_parts('${SubaccountTable.subaccountIdToUuid(orderId.subaccountId!)}', '${orderId.clientId}', '${orderId.clobPairId}', '${orderId.orderFlags}') AS result`);
    expect(result).toEqual(OrderTable.orderIdToUuid(orderId));
  });
  ...
  ])('jinx_uuid_from_perpetual_position_parts (%s, %s, %s, %s)', async (subaccountId: IndexerSubaccountId, blockHeight: number, transactionIndex: number, eventIndex: number) => {
    const subaccountUuid = SubaccountTable.subaccountIdToUuid(subaccountId);
    const eventId = TendermintEventTable.createEventId(`${blockHeight}`, transactionIndex, eventIndex);
    const result = await getSingleRawQueryResultRow(
      `SELECT jinx_uuid_from_perpetual_position_parts('${subaccountUuid}', '\\x${eventId.toString('hex')}'::bytea) AS result`);
  ...
  ])('jinx_uuid_from_subaccount_id and parts (%s)', async (subaccountId: IndexerSubaccountId) => {
    let result = await getSingleRawQueryResultRow(`SELECT jinx_uuid_from_subaccount_id('${JSON.stringify(subaccountId)}') AS result`);
    expect(result).toEqual(SubaccountTable.subaccountIdToUuid(subaccountId));

    result = await getSingleRawQueryResultRow(`SELECT jinx_uuid_from_subaccount_id_parts('${subaccountId.owner}', '${subaccountId.number}') AS result`);
    expect(result).toEqual(SubaccountTable.subaccountIdToUuid(subaccountId));
  });
  ...
  it.each([
  ...
  ])('jinx_uuid_from_transfer_parts (%s, %s, %s, %s)', async (
    ...
    const result = await getSingleRawQueryResultRow(
      `SELECT jinx_uuid_from_transfer_parts('\\x${eventId.toString('hex')}'::bytea, '${assetId}', ${senderSubaccountUuid ? `'${senderSubaccountUuid}'` : 'NULL'}, ${recipientSubaccountUuid ? `'${recipientSubaccountUuid}'` : 'NULL'}, ${senderWalletAddress ? `'${senderWalletAddress}'` : 'NULL'}, ${recipientWalletAddress ? `'${recipientWalletAddress}'` : 'NULL'}) AS result`);
  ...
  ])('jinx_tendermint_event_to_transaction_index should return the expected result', async (
    { event, expectedResult, expectedError },
  ) => {
    try {
      const result = await getSingleRawQueryResultRow(
        `SELECT jinx_tendermint_event_to_transaction_index('${JSON.stringify(event)}') AS result`);
      if (expectedError) {
  ...
  ])('jinx_uuid_from_transaction_parts (%s, %s)', async (blockHeight: string, transactionIndex: number) => {
    const result = await getSingleRawQueryResultRow(
      `SELECT jinx_uuid_from_transaction_parts('${blockHeight}', '${transactionIndex}') AS result`);
  ...
  ])('jinx_uuid_from_oracle_price_parts (%s, %s)', async (marketId: number, blockHeight: string) => {
    const result = await getSingleRawQueryResultRow(
      `SELECT jinx_uuid_from_oracle_price_parts('${marketId}', '${blockHeight}') AS result`);
    expect(result).toEqual(
      OraclePriceTable.uuid(marketId, blockHeight),
    );
  });

  it('jinx_clob_pair_status_to_market_status should convert all statuses', async () => {
    for (const [key, value] of Object.entries(CLOB_STATUS_TO_MARKET_STATUS)) {
      const result = await getSingleRawQueryResultRow(
        `SELECT jinx_clob_pair_status_to_market_status('${key}') AS result`);
      expect(result).toEqual(value);
    }
  });

  it('jinx_create_transaction.sql should insert a transaction and return correct jsonb', async () => {
    const transactionHash: string = 'txnhash';
    const blockHeight: string = '1';
    const transactionIndex: number = 123;

    const returnedJsonb = await getSingleRawQueryResultRow(`SELECT jinx_create_transaction('${transactionHash}', '${blockHeight}', ${transactionIndex}) AS result`);
  ...
  it('jinx_create_tendermint_event.sql should insert a tendermint event and return correct jsonb', async () => {
    ...
    const result = await getSingleRawQueryResultRow(
      `SELECT jinx_create_tendermint_event('${JSON.stringify(indexerTendermintEvent)}', '${testConstants.defaultBlock.blockHeight}') AS result`,
    );
  ...
  it('jinx_create_initial_rows_for_tendermint_block.sql should insert the initial rows correctly', async () => {
    const blockHeight = '1';
    const txHashes = [defaultTxHash, defaultTxHash2];
    const dateTimeIso = '2020-01-01T00:00:00.000Z';
    await getSingleRawQueryResultRow(
      `SELECT jinx_create_initial_rows_for_tendermint_block(${blockHeight}, '${dateTimeIso}', '${JSON.stringify(txHashes)}', '${JSON.stringify(events)}')`,
    );
