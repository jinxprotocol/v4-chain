...
export class QueryClientImpl implements Query {
  ...
  perpetual(request: QueryPerpetualRequest): Promise<QueryPerpetualResponse> {
    ...
    const promise = this.rpc.request("jinxprotocol.perpetuals.Query", "Perpetual", data);
  ...
  allPerpetuals(request: QueryAllPerpetualsRequest = {
    ...
    const promise = this.rpc.request("jinxprotocol.perpetuals.Query", "AllPerpetuals", data);
  ...
  allLiquidityTiers(request: QueryAllLiquidityTiersRequest = {
    ...
    const promise = this.rpc.request("jinxprotocol.perpetuals.Query", "AllLiquidityTiers", data);
  ...
  premiumVotes(request: QueryPremiumVotesRequest = {}): Promise<QueryPremiumVotesResponse> {
    ...
    const promise = this.rpc.request("jinxprotocol.perpetuals.Query", "PremiumVotes", data);
  ...
  premiumSamples(request: QueryPremiumSamplesRequest = {}): Promise<QueryPremiumSamplesResponse> {
    ...
    const promise = this.rpc.request("jinxprotocol.perpetuals.Query", "PremiumSamples", data);
    return promise.then(data => QueryPremiumSamplesResponse.decode(new _m0.Reader(data)));
  }

  params(request: QueryParamsRequest = {}): Promise<QueryParamsResponse> {
    const data = QueryParamsRequest.encode(request).finish();
    const promise = this.rpc.request("jinxprotocol.perpetuals.Query", "Params", data);
