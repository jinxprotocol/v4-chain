...
2. Build the app binary (i.e. `jinxprotocold`)
...
TODO(CORE-512): add info/resources around cloud setup. [Doc](https://www.notion.so/jinx/Cloud-Setup-ccc68e7b6a4b4e83a8d0c35029a9997f)
...
```sh
# dev
docker build . --progress=plain --no-cache -f ./testing/testnet-dev/Dockerfile -t testnet && docker run testnet start --home /jinxprotocol/chain/.alice

# staging
docker build . --progress=plain --no-cache -f ./testing/testnet-staging/Dockerfile -t testnet && docker run testnet start --home /jinxprotocol/chain/.alice
```
...
example: `AWS_PROFILE=jinx-v4 ./scripts/build-push-ecr.sh staging`
...
If you wish to run as a full-node instead of a validator, specify the `--home` flag as ` /jinxprotocol/chain/.full-node`.

```sh
$ docker build . --progress=plain --no-cache -f ./testing/testnet-dev/Dockerfile -t testnet && docker run testnet start --home /jinxprotocol/chain/.full-node
```
