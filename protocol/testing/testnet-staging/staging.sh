...
CHAIN_ID="jinxprotocol-testnet"

# Define mnemonics for all validators.
MNEMONICS=(
	# alice
	# Consensus Address: jinxvalcons1zf9csp5ygq95cqyxh48w3qkuckmpealrw2ug4d
	...
	# bob
	# Consensus Address: jinxvalcons1s7wykslt83kayxuaktep9fw8qxe5n73ucftkh4
	...
	# carl
	# Consensus Address: jinxvalcons1vy0nrh7l4rtezrsakaadz4mngwlpdmhy64h0ls
	...
	# dave
	# Consensus Address: jinxvalcons1stjspktkshgcsv8sneqk2vs2ws0nw2wr272vtt
	...
	# emily
	# Consensus Address: jinxvalcons1zpt0ck6ttuhjm97apawa54ffgthst34u3peqc0
	...
	# fiona
	# Consensus Address: jinxvalcons14wwueldgtrdjrmx23wcuwk83keywe5w0pfn94t
	...
	# greg
	# Consensus Address: jinxvalcons15yzv3qacs0z2jgm5ecn4ywjkvwc6dl63a2za0p
	...
	# henry
	# Consensus Address: jinxvalcons1pggt0hc2drw0j9456vwpu9wmav67h90h03p3h9
	...
	# ian
	# Consensus Address: jinxvalcons167ajkznjs3wfa565n9emqmey0c2h69ympf0tmk
	...
	# jeff
	# Consensus Address: jinxvalcons1ehwtcwwh25ftac3khhz0jn7wd9053xfeqdwes4
...
# This also controls in which directory the validator's home will be located. i.e. `/jinxprotocol/chain/.alice`
MONIKERS=(
	"alice"
	"bob"
	"carl"
...
TEST_ACCOUNTS=(
	"jinx199tqg4wdlnu4qjlxchpd7seg454937hjrknju4" # alice
	"jinx10fx7sy6ywd5senxae9dwytf8jxek3t2gcen2vs" # bob
	"jinx1fjg6zp6vv8t9wvy4lps03r5l4g7tkjw9wvmh70" # carl
	"jinx1wau5mja7j7zdavtfq9lu7ejef05hm6ffenlcsn" # dave
	"jinx1966p9acs4mpgj40g3awctfvgrz0mnx8w5vc4sz" # emily
	"jinx18swhz9sgh8ecjaz3cm0v53llw0qzm5se026rsn" # fiona
	"jinx1df84hz7y0dd3mrqcv3vrhw9wdttelul8edqmvp" # greg
	"jinx16h7p7f4dysrgtzptxx2gtpt5d8t834g9dj830z" # henry
	"jinx15u9tppy5e2pdndvlrvafxqhuurj9mnpdstzj6z" # ian
	"jinx168pjt8rkru35239fsqvz7rzgeclakp49zx3aum" # jeff
)

FAUCET_ACCOUNTS=(
	"jinx1nzuttarf5k2j0nug5yzhr6p74t9avehn9hlh8m" # main faucet
	"jinx10du0qegtt73ynv5ctenh565qha27ptzr6dz8c3" # backup #1
	"jinx1axstmx84qtv0avhjwek46v6tcmyc8agu03nafv" # backup #2
)
...
create_validators() {
	...
	for i in "${!FULL_NODE_KEYS[@]}"; do
		...
		jinxprotocold init "full-node" -o --chain-id=$CHAIN_ID --home "$FULL_NODE_HOME_DIR"
	...
	for i in "${!MONIKERS[@]}"; do
		...
		jinxprotocold init "${MONIKERS[$i]}" -o --chain-id=$CHAIN_ID --home "$VAL_HOME_DIR"

		# Overwrite the randomly generated `priv_validator_key.json` with a key generated deterministically from the mnemonic.
		jinxprotocold tendermint gen-priv-key --home "$VAL_HOME_DIR" --mnemonic "${MNEMONICS[$i]}"
		...
		echo "${MNEMONICS[$i]}" | jinxprotocold keys add "${MONIKERS[$i]}" --recover --keyring-backend=test --home "$VAL_HOME_DIR"
		...
		for acct in "${TEST_ACCOUNTS[@]}"; do
			jinxprotocold add-genesis-account "$acct" 100000000000000000$USDC_DENOM,$TESTNET_VALIDATOR_NATIVE_TOKEN_BALANCE$NATIVE_TOKEN --home "$VAL_HOME_DIR"
		done
		for acct in "${FAUCET_ACCOUNTS[@]}"; do
			jinxprotocold add-genesis-account "$acct" 900000000000000000$USDC_DENOM,$TESTNET_VALIDATOR_NATIVE_TOKEN_BALANCE$NATIVE_TOKEN --home "$VAL_HOME_DIR"
		done

		jinxprotocold gentx "${MONIKERS[$i]}" $TESTNET_VALIDATOR_SELF_DELEGATE_AMOUNT$NATIVE_TOKEN --moniker="${MONIKERS[$i]}" --keyring-backend=test --chain-id=$CHAIN_ID --home "$VAL_HOME_DIR"
	...
	jinxprotocold collect-gentxs --home "$FIRST_VAL_HOME_DIR"
...
setup_cosmovisor() {
	for i in "${!FULL_NODE_KEYS[@]}"; do
		FULL_NODE_HOME_DIR="$HOME/chain/.full-node-$i"
		export DAEMON_NAME=jinxprotocold
		export DAEMON_HOME="$HOME/chain/.full-node-$i"

		cosmovisor init /bin/jinxprotocold
	done
