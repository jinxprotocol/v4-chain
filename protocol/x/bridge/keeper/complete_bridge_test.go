...
func TestCompleteBridge(t *testing.T) {
	tests := map[string]struct {
		...
		"Success": {
			initialModAccBalance:  sdk.NewCoin("jinx", sdkmath.NewInt(1_000)),
			bridgeEvent:           constants.BridgeEvent_Id0_Height0,           // bridges 888 tokens.
			expectedModAccBalance: sdk.NewCoin("jinx", sdkmath.NewInt(112)), // 1000 - 888.
		},
		"Success: coin amount is 0": {
			...
			bridgeEvent: types.BridgeEvent{
				...
				Coin:           sdk.NewCoin("jinx", sdkmath.ZeroInt()),
				EthBlockHeight: 3,
			},
			expectedModAccBalance: sdk.NewCoin("jinx", sdkmath.NewInt(1_000)),
		},
		...
		"Failure: bridge module account has insufficient balance": {
			initialModAccBalance:  sdk.NewCoin("jinx", sdkmath.NewInt(500)),
			bridgeEvent:           constants.BridgeEvent_Id0_Height0, // bridges 888 tokens.
			expectedError:         "insufficient funds",
			expectedModAccBalance: sdk.NewCoin("jinx", sdkmath.NewInt(500)),
		},
		"Failure: bridging is disabled": {
			...
			expectedModAccBalance: sdk.NewCoin("jinx", sdkmath.NewInt(1_000)), // same as initial.
		},
	}
